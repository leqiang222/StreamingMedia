...
/**
 *  当外界, 需要播放一段音频资源时候, 会抛出一个请求, 给这个对象
 *  这个对象, 到时候, 只需要根据请求信息, 抛数据给外界
 */
- (BOOL)resourceLoader:(AVAssetResourceLoader *)resourceLoader shouldWaitForLoadingOfRequestedResource:(AVAssetResourceLoadingRequest *)loadingRequest {
    NSLog(@"/remoteAudio/-------- 资源的请求: %@ --------/remoteAudio/", loadingRequest);
...
- (void)resourceLoader:(AVAssetResourceLoader *)resourceLoader didCancelLoadingRequest:(AVAssetResourceLoadingRequest *)loadingRequest {
    NSLog(@"/remoteAudio/-------- 取消某个请求 --------/remoteAudio/");
    [self.loadingRequests removeObject:loadingRequest];
}

#pragma mark - JCAudioDownLoaderDelegate
...
- (void)handleAllLoadingRequest {
    NSLog(@"/remoteAudio/-------- 当前请求集合: %@ --------/remoteAudio/", self.loadingRequests);
...
- (void)handleLoadingRequest:(AVAssetResourceLoadingRequest *)loadingRequest {
    ...
    // 计算总大小
    NSURL *url = loadingRequest.request.URL;
    long long totalSize = [JCAudioFileTool cacheFileSizeWithURL:url];
    loadingRequest.contentInformationRequest.contentLength = totalSize;
    
    NSString *contentType = [JCAudioFileTool contentTypeWithURL:url];
    loadingRequest.contentInformationRequest.contentType = contentType;
    loadingRequest.contentInformationRequest.byteRangeAccessSupported = YES;
